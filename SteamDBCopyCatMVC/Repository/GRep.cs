...
{
    public class GRep<Tbl_Entity> : IRep1<Tbl_Entity> where Tbl_Entity : class
    {
        DbSet<Tbl_Entity> _dbSet;

        private SteamDBCopyCatEntities _DBEntity;

        public GRep(SteamDBCopyCatEntities DBEntity)
        {
            _DBEntity = DBEntity;
        ...
        public void Remove(int recordId)
        {
            ...
            Tbl_Entity theTable = _dbSet.Find(recordId);
            if (_DBEntity.Entry(theTable).State == EntityState.Detached)
                _dbSet.Attach(theTable);
            _dbSet.Remove(theTable);
            _DBEntity.SaveChanges();
        }
        ...
        public IEnumerable<Tbl_Entity> GetProductById()
        {
